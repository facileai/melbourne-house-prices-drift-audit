...
def get_baseline_target_range(
    ...
    Set baseline as all data up to 6 months prior, and target as the most recent 6 months.
    ...
    if baseline_end == "":
        baseline_end = range_max - relativedelta(months=+6)
    ...
    if target_start == "":
        target_start = range_max - relativedelta(months=+6)
...
def detect_drift_by_ID(
    ...
    for group_value in group_values:
        ...
        try:
            ...
            drift_by_feature = rank_feature_drift(preds_h0, features,p_val=p_val)
...
if __name__ == "__main__":
    ...
    end_date = datetime.datetime.strptime(target_end, '%Y-%m-%d')
    start_date = datetime.datetime.strptime(target_start, '%Y-%m-%d')
    num_months = (end_date.year - start_date.year) * 12 + (end_date.month - start_date.month)
    
    for i in range(num_months+1):
        target_start =  (start_date+relativedelta(months=+i)).strftime('%Y-%m-%d')
        target_end = (start_date+relativedelta(months=+i+1)).strftime('%Y-%m-%d')
        
        drift_results_df_partial = detect_drift_by_ID(
            group_col=group_col,
        ...
        drift_results_df = pd.concat([drift_results_df, drift_results_df_partial])
    # ------------------------------------
    # 4. Write results
    # ------------------------------------
    drift_results_df['target_date'] = drift_results_df['target_date'].apply(pd.to_datetime)
    results_file_name = "model-{0}_distribution_drift_results.csv".format(modelID)
    drift_results_df.to_csv(f'./data/{results_file_name}', index=False, header=True)
